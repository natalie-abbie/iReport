...
class TestUser(unittest.TestCase):
    ...
    def test_UserInstance(self):
        self.userObj1 = User(1, 'abio', 'natalie','nats','talie','abionatline@gmail.com','0752030815', generate_password_hash('password'),'true')
    ...
    def test_user_login_failed_wrong_password(self):        
        ...
        data = json.loads(response.data)
        self.assertIn('unauthorised access, invalid username or password', data['message'])        
        self.assertEqual(401, response.status_code)
...
class TestRedflag(unittest.TestCase):
    ...
    def setUp(self):
        self.client = app.test_client()
        self.flagObj = Redflag(1,1, 'Bribery','traffic police officer being bribere with money', 'abionatline@gmail.com', 'kyanja','nats')
    ...
    def test_short_description_create_redflag(self):
        ...
        data = json.loads(response.data)
        self.assertEqual('name of flag should be well defined', data['message'])
        self.assertEqual(400, response.status_code)
    ...
    def test_delete_flag_failed(self):
        response = self.client.delete('/api/v1/redflag/<flag_id>', content_type='application/json')
        data = json.loads(response.data)
        self.assertEqual('No flag has that id, nothing was deleted', data['message'])
        self.assertEqual(400, response.status_code)
     

    def test_update_flag_successful(self):
        response = self.client.put('api/v1/redflag/<flag_id>', content_type='application/json', data=json.dumps({'name':'flagdemo', 'location':'', 'category':'', 'description':''}))
