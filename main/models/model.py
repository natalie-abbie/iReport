...
class Redflag:
    ...
    def create_redflag(self):
        ...
        oldflagListLength = len(FLAGS) # length of redflag list before manipulation.
        #create the list below with precise indexing as illustrated below
        # [0] = user_id, [1] = type, [2] = description, [3] = email, [4] = location, [5] =createdon [6] = created by
        FLAGS.append({self.flag_id:[self.type, self.user_id, self.description, self.email, self.location,self.createdOn, self.createdby]})
        
        if len(FLAGS) > oldflagListLength:
            result = True #incase creating a redflag is successful return true.
        else:
